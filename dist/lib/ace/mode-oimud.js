ace.define("ace/mode/oimud_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (require, exports, module) {
    ...
    oop.inherits(oimudHighlightRules, TextHighlightRules);

    exports.oimudHighlightRules = oimudHighlightRules;
});

ace.define("ace/mode/folding/oimud", ["require", "exports", "module", "ace/lib/oop", "ace/range", "ace/mode/folding/fold_mode"], function (require, exports, module) {
    "use strict";
...
ace.define("ace/mode/oimud", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/oimud_highlight_rules", "ace/mode/folding/oimud"], function (require, exports, module) {
    "use strict";

    var oop = require("../lib/oop");
    var TextMode = require("./text").Mode;
    var oimudHighlightRules = require("./oimud_highlight_rules").oimudHighlightRules;
    var CStyleFoldMode = require("./folding/oimud").FoldMode;

    var Mode = function () {
        this.HighlightRules = oimudHighlightRules;
    ...
    var docs = {
        ...
        'zcolor': { text: '<b>zcolor(code)</b><br/>converts a zmud/cmud color code into a code supported by oimud', type: 4 },
        'ansi': { text: '<b>ansi(style,fore,back)</b><br/>insert ansi control characters into string same as ${esc}[CODESm<ul><li><b>style</b>the styles to apply, <i>optional</i><ul><li><b>reset</b>reset all styles andcolors</li><li>bold,faint,italic,underline,blink,reverse,hidden,strikethrough,doubleunderline,boldoff,italicoff,blinkoff,blinkrapidoff,visible,strikethroughoff</li></ul></li><li><b>fore</b>the ground color, if bold and a valid colored bold is considered the foreground color, may also be default</li><li><b>back</b>the background color, may be default <i>optional</i></li><li><ul><li><b>oimud custom colors: localecho, infotext, infobackground, errortext<li><b>Colors: red, blue, orange, yellow, green, black, white, cyan, magenta, gray<ul><li>prepend with x for aixterm bright colors</li><li>append background to get background code directly</li><li>eg redbackground to get red background, xred to get aixterm red or xredbackground to get aixterm red background</li></ul></li></ul>', type: 4 },
        ...
        'COLOR': { text: '<b><u>#CO</u>LOR <i>{pattern}</i> fore,back,bold <i>profile</i></b><br>Color last added line, comma delimited colors, supports raw oimud color codes, ansi word values, any valid HTML color name, of ##RRGGBB html format', type: 2 },
        'CO': { text: '<b><u>#CO</u>LOR <i>{pattern}</i> fore,back,bold <i>profile</i></b><br>Color last added line, comma delimited colors, supports raw oimud color codes, ansi word values, any valid HTML color name, of ##RRGGBB html format', type: 2 },
        ...
        'NOTIFY': { text: '<b><u>#NOT</u>IFY title message <i>{icon}</i></b><br/>display a notification popup with no sound, use [client.notify](scriptind.md#basic-function-list) to turn off silent option or #playsound', type: 2 },
        'NOT': { text: '<b><u>#NOT</u>IFY title message <i>{icon}</i></b><br/>display a notification popup with no sound, use [client.notify](scriptind.md#basic-function-list) to turn off silent option or #playsound', type: 2 },
        ...
        'VERSION': { text: '<b><u>#VE</u>RSION</b><br/>Display current oimud version information', type: 2 },
        'VE': { text: '<b><u>#VE</u>RSION</b><br/>Display current oimud version information', type: 2 },
        ...
        'TESTSPEEDFILE': { text: '<b>#TESTSPEEDFILE</b><br/>Works exactly like #testspeed but will open file dialog asking for a file to test with instead of built in test functions, **Note** file load time can cause test to run longer then total time returned', type: 2 },
        'TESTSPEEDFILER': { text: '<b>#TESTSPEEDFILER</b><br/>Works exactly like #TestSpeedFile but will attempt to emulate as if sent from remote mud for processing', type: 2 },
        'TESTFILE': { text: '<b>#TESTFILE</b><br/>Opens a file dialog to be read, displays it and time to display **Note** this will cause the client to become unresponsive or locked while running this test, either wait til done or close.', type: 2 },
        'TESTFILER': { text: '<b>#TESTFILER</b><br/>Works exactly like #TESTFILE but will attempt to emulate as if sent from remote mud for processing', type: 2 },
        ...
        'TESTSTATUS': { text: '<b>#TESTSTATUS <i></i>party:#</i> <i>monter:#</i> <i>night</i></b><br/>Send GMCP status packages to simiulate data from a mud, passing party/monster:# argument will craete bars from 1 to #, may set night mode by passing night argument.', type: 2 },
        'TESTSCREEN': { text: '<b>#TESTSCREEN</b><br/>Display data about current screen dimensions', type: 2 },
        ...
        'PROMPT': { text: '<b><u>#PR</u>OMPT variable <i>message defaultValue mask</i></b><br/>Displays a prompt dialog to enter text and store results in variable, if canceled or closed result is null', type: docType.command },
        'PR': { text: '<b><u>#PR</u>OMPT variable <i>message defaultValue mask</i></b><br/>Displays a prompt dialog to enter text and store results in variable, if canceled or closed result is null', type: docType.command },
        ...
        'prompt': { text: '<b>prompt(<i>prompt</i>,<i>defaultValue</i>,<i>mask</i>)</b><br/>Displays a prompt dialog to enter text and return the value, if canceled or closed result is null or empty string', type: docType.function }
    };
    (function () {
        this.$id = "ace/mode/oimud";
    ...
    ace.require(["ace/mode/oimud"], function (m) {
